...
const EducationList = () => {
  ...
  const fetchItems = () =>
    API.get('/education')
      .then(r => setItems(toArray(r.data, 'education', 'data')))
      .catch(() => {});
    ...
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="page-subtitle">Your academic background and qualifications</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: '20px' }}>
          <div className="card-header">
            <span className="card-title">{editing ? 'Edit Record' : 'New Education'}</span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid form-grid-2">
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input name="institution" value={form.institution} onChange={handleChange} placeholder="e.g. IIT Bombay" className="form-input" required />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label className="form-label">Course / Degree</label>
                  <input name="course" value={form.course} onChange={handleChange} placeholder="e.g. B.Tech in Computer Science" className="form-input" required />
                </div>
                <div className="form-group">
                  <label className="form-label">Duration</label>
                  <input name="duration" value={form.duration} onChange={handleChange} placeholder="e.g. 2020 – 2024" className="form-input" required />
                </div>
                <div className="form-group">
                  <label className="form-label">CGPA (optional)</label>
                  <input name="cgpa" value={form.cgpa} onChange={handleChange} placeholder="e.g. 8.7" className="form-input" />
                </div>
                <div className="form-group">
                  <label className="form-label">Percentage (optional)</label>
                  <input name="percentage" value={form.percentage} onChange={handleChange} placeholder="e.g. 87%" className="form-input" />
                </div>
              </div>
          ...
          <div key={item._id} className="list-item">
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1 }}>
              <div style={{
                ...
                fontWeight: '700', color: '#3B82F6', fontFamily: 'DM Sans, sans-serif',
              }}>{idx + 1}</div>
              <div>
