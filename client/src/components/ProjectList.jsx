...
const ProjectList = () => {
  ...
  const fetchItems = () =>
    API.get('/projects')
      .then(r => setItems(toArray(r.data, 'projects', 'data')))
      .catch(() => {});
  ...
  const handleTechRemove = (i) =>
    setForm({ ...form, tech: (form.tech || []).filter((_, idx) => idx !== i) });
    ...
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="page-subtitle">Showcase your best work and side projects</p>
        </div>
        {!showForm && (
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid form-grid-2">
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input name="title" value={form.title} onChange={handleChange} placeholder="e.g. E-Commerce Platform" className="form-input" required />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div className="inline-add">
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleTechAdd(); } }}
                  ...
                  {(form.tech || []).length > 0 && (
                    <div className="tags-wrap">
                      {form.tech.map((t, i) => (
                        <span key={i} className="tag" style={{ background: '#EFF6FF', borderColor: '#BFDBFE', color: '#1D4ED8' }}>
                          {t}
                          <button type="button" onClick={() => handleTechRemove(i)} className="tag-remove">×</button>
                        </span>
                      ))}
                    </div>
                  )}
                ...
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea name="description" value={form.description} onChange={handleChange} placeholder="Describe what this project does…" className="form-input" required />
                </div>
                <div className="form-group">
                  <label className="form-label">Demo URL</label>
                  <input name="demo" value={form.demo} onChange={handleChange} placeholder="https://…" className="form-input" />
                </div>
                <div className="form-group">
                  <label className="form-label">Repository URL</label>
                  <input name="repo" value={form.repo} onChange={handleChange} placeholder="https://github.com/…" className="form-input" />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <div style={{ display: 'flex', gap: '9px', alignItems: 'center' }}>
                <button type="submit" className="btn btn-success" disabled={submitting}>
                  {submitting ? 'Saving…' : editing ? '✓ Update' : '+ Add Project'}
                </button>
      ...
      <div style={{ display: 'grid', gap: '10px' }}>
        {items.length === 0 && !showForm && (
          <div className="card">
            <div className="card-body" style={{ textAlign: 'center', padding: '48px', color: '#A1A1AA' }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '5px' }}>
                  {(Array.isArray(item.tech) ? item.tech : []).map((t, i) => (
                    <span key={i} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '99px', background: '#EFF6FF', border: '1px solid #BFDBFE', color: '#1D4ED8', fontFamily: 'DM Sans, sans-serif', fontWeight: '500' }}>{t}</span>
                  ))}
                </div>
                <p style={{ fontSize: '12.5px', color: '#71717A', fontFamily: 'DM Sans, sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
