...
const CertificationList = () => {
  ...
  const fetchItems = () =>
    API.get('/certifications')
      .then(r => setItems(toArray(r.data, 'certifications', 'data')))
      .catch(() => {});
  ...
  const handleEdit = (item) => {
    setEditing(item._id);
    setForm({ name: item.name, issuer: item.issuer, image: item.image, link: item.link });
    setImageFile(null);
    setImagePreview('');
    setShowForm(true);
  };
    ...
    <div>
      ...
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="page-subtitle">Licenses, badges, and professional credentials</p>
        </div>
        {!showForm && (
          <button className="btn btn-primary" onClick={() => setShowForm(true)}>+ Add Certification</button>
        )}
      </div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: '20px' }}>
          <div className="card-header">
            <span className="card-title">{editing ? 'Edit Certification' : 'New Certification'}</span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid form-grid-2">
                <div className="form-group">
                  ...
                  <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. AWS Solutions Architect" className="form-input" required />
                </div>
                <div className="form-group">
                  <label className="form-label">Issuing Organization</label>
                  <input name="issuer" value={form.issuer} onChange={handleChange} placeholder="e.g. Amazon Web Services" className="form-input" required />
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '14px' }}>
        ...
        {items.map(item => (
          <div key={item._id} className="card" style={{ overflow: 'hidden' }}>
            {item.image
              ? <div style={{ height: '130px', overflow: 'hidden', background: '#F4F4F5' }}>
            ...
            <div style={{ padding: '14px 16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                {item.link
                  ? <a href={item.link} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px', color: '#3B82F6', fontFamily: 'DM Sans, sans-serif', textDecoration: 'none', fontWeight: '500' }}>View →</a>
                  : <span />
                }
